...
**Deferred / not yet done — in priority order (read before continuing work; reviewed 2026-07-09 now that real beta testers are being onboarded):**
1. **`src/proxy.ts` still calls `supabase.auth.getUser()` on every navigation** — a live network round-trip to Supabase's auth server on every click, the main source of felt lag. The fix (swap to `getSession()`, which reads the session from the cookie locally with no network call) was scoped and explicitly deferred at the user's request until closer to production — that point is closer now. RLS still protects all data regardless of which check is used. Note: `getUser()` is also the more *secure* of the two (it validates the JWT against Supabase rather than trusting an unverified local cookie), so this is a deliberate security/speed tradeoff to make consciously, not a pure cleanup.
2. **No account deletion or data export.** Real testers with real emails and real workout data currently have no way to delete their account or export their data — worth adding before wider rollout.
3. **No CAPTCHA on `/register`.** Originally skipped because there was "no public signup traffic yet" (see Security hardening below) — that assumption is weaker now that beta testers are being recruited. `/register` is open to anyone with the URL, not just invited testers, since the beta signup Google Form is just a courtesy collection step, not an actual gate.
4. **No error monitoring in production.** If something breaks for a tester mid-session there's currently no way to find out unless they report it directly — no Sentry or equivalent wired up.
5. **No privacy policy or terms page.** Minor for a small beta among people the user knows personally, but worth a one-line page given real emails + personal training data are being collected.
6. **"Prevent use of leaked passwords" (HaveIBeenPwned check) is Pro-plan-only on Supabase** — not available on this project's Free tier without a billing upgrade. Nothing actionable here unless/until upgrading — **lowest priority, do this last.**
