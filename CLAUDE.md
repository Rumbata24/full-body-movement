...
- **CAPTCHA** (Cloudflare Turnstile) — deliberately **not** added; judged unnecessary friction for a single-user app with no public signup traffic yet. That assumption is weaker now that beta testers are being recruited (see the prioritized deferred-items list in "Current architecture notes" below). Attack Protection page has a ready-made toggle for it once a Turnstile site key exists.
...
**Deferred / not yet done — in priority order (reviewed 2026-07-12):**
...
2. ✅ **CAPTCHA (Cloudflare Turnstile)** — code done 2026-07-13, but **not live yet** — needs one manual step from the user. `src/components/ui/Turnstile.tsx` is a shared widget (loads Cloudflare's script, renders the challenge, reports the token via `onVerify`), wired into `/register`, `/login`, and `/forgot-password` (all three, not just register — Supabase's "Enable Captcha protection" toggle applies project-wide to sign-up/sign-in/password-reset, not per-route, so leaving login unprotected would either do nothing or break login once the toggle flips on). Site key lives in `NEXT_PUBLIC_TURNSTILE_SITE_KEY` (public by design, safe to commit to env). Turnstile widget only has the production hostname (`calisthenics-skill-tracker.vercel.app`) allow-listed — `localhost` isn't a valid Turnstile hostname (no TLD), so the widget deliberately shows a Cloudflare "Unable to connect to website" error in local dev; this is expected, not a bug — the buttons stay disabled until a token exists so this only blocks testing the real submit flow locally, not correctness. Required adding `https://challenges.cloudflare.com` to both `script-src` and a new `frame-src` directive (the challenge renders in its own iframe) in `src/proxy.ts`'s CSP — same "new third-party endpoint needs a CSP allowance or it silently fails" lesson as the Sentry item above. **Still to do (the user must do these, not Claude — security-settings changes are out of scope for the agent):** (a) add `NEXT_PUBLIC_TURNSTILE_SITE_KEY` to Vercel env vars, (b) in Supabase Dashboard → Authentication → Attack Protection, enable CAPTCHA protection with provider Turnstile and paste in the **secret key** (never given to Claude or committed anywhere — user holds it exclusively).
3. ✅ **Production error monitoring** — done 2026-07-12. `@sentry/nextjs` wired up via `src/instrumentation.ts` + `src/instrumentation-client.ts` + `sentry.server.config.ts`/`sentry.edge.config.ts` (all read `NEXT_PUBLIC_SENTRY_DSN`; DSN isn't secret, safe to be public), plus `src/app/global-error.tsx` to catch React render errors. `tracesSampleRate: 0` — error capture only, no performance tracing (kept deliberately minimal; no source-map upload / `withSentryConfig` wrapper since that needs a Sentry auth token we don't have yet — stack traces work but point at built/minified code until that's added). Required adding `https://*.sentry.io` to `connect-src` in `src/proxy.ts`'s CSP, or the browser silently blocks the report with no console error (same class of issue as the CSP nonce gotcha below — always check `connect-src` when adding a new third-party endpoint). Verified live: triggered both an unhandled error and an explicit `captureException`, confirmed both landed in the Sentry issues feed. **`NEXT_PUBLIC_SENTRY_DSN` still needs to be added to Vercel's env vars** (Production + Preview) — it's only in local `.env.local` so far.
...
5. **"Prevent use of leaked passwords" (HaveIBeenPwned check) is Pro-plan-only on Supabase** — not available on this project's Free tier without a billing upgrade. Nothing actionable here unless/until upgrading — **lowest priority, do this last.**
