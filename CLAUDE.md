...
- `src/app/(app)/profile/page.tsx` — view/edit `display_name` and `units` (backed by `getProfile`/`updateProfile` in `src/lib/data.ts`), shows the account email (read-only, from the auth user), and a sign-out button. Linked from a small circular icon button in the Today screen header (top-right).
- `src/app/auth/callback/route.ts` — no longer used by the current login/register flow (no more magic links or confirmation emails). Left in place since a future "forgot password" flow (see Deferred list) would likely reuse the same PKCE `code`-exchange pattern.
- `src/proxy.ts` — `/register` added to `PUBLIC_PATHS`; signed-in users hitting `/login` or `/register` are redirected to `/`.
...
- `src/proxy.ts` — Next 16's `middleware.ts` replacement; gates all routes except `/login`, `/register`, `/auth`, `/offline`.
...
- No "forgot password" flow yet — was out of scope for the initial email+password build. Add via `supabase.auth.resetPasswordForEmail()` + a reset-password page if users start getting locked out.
